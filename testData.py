...
'''
depList = glob.glob("../RST_annotation_results/RST_Annotation_Results/coherent_texts_DEP/*.dep")
for each in depList:
    g = loadDiscTree(each)
    g.parse(doPlot=True, show=False, fName=each.split(".dep")[0] + ".png")
    depList = glob.glob("../RST_annotation_results/RST_Annotation_Results/incoherent_texts_DEP/*.dep")
for each in depList:
    g = loadDiscTree(each)
    g.parse(doPlot=True, show=False, fName=each.split(".dep")[0] + ".png")
'''
fName = "../RST_annotation_results/A1_FLTRP_2013_A_027_OSPL.txt.edu.dep"
g = loadDiscTree(fName)
g.parse(show=False, fName=fName.split(".dep")[0] + ".png")
'''
